...
while err>0.000000000001
    umaxprev=max(max(u(2:N,2:N)));
    iter=iter+1;
    %x varies from 2 to N as values at 1 and N+1 are fixed
    for i=2:N
        for j=2:N
            u(i,j)=(1-w)*(-F(i,j)*h2-(u(i+1,j)+u(i-1,j)+u(i,j+1)+u(i,j-1)))/-4+w*(-F(i,j)*h2-(u(i+1,j)+u(i-1,j)+u(i,j+1)+u(i,j-1)))/-4;
        end
        %Neumann B.C.
        %To acheive du/dy=0, ghost node at node -1 is the same value as
        %node 2
        j=1;
        u(i,j)=(1-w)*(-F(i,j)*h2-(u(i+1,j)+u(i-1,j)+2*u(i,j+1)))/-4+w*(-F(i,j)*h2-(u(i+1,j)+u(i-1,j)+2*u(i,j+1)))/-4;
    end
...
surf(x,y,u)
xlabel('X'),ylabel('Y'),zlabel('U(X,Y)'), title('SSOR Gauss-Seidel Solution')